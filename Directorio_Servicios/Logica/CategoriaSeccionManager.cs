...
{
    public class CategoriaSeccionManager
    {
        ...
        public CategoriaSeccionManager(TabControl tabControl, FlowLayoutPanel flpDenominaciones,
        ...
        public void MostrarSeccionCategoria(Categoria categoria)
        {
            ...
            _tabControl.SelectedTab = _tabControl.TabPages[1]; // Segunda pestaña
        ...
        public void ConfigurarBuscador(TextBox txtBuscar, Button btnBuscar)
        {
            txtBuscar.Enter += (s, e) =>
            {
                if (txtBuscar.Text == "BUSCAR SERVICIOS")
                {
                    txtBuscar.Text = "";
            ...
            txtBuscar.Leave += (s, e) =>
            {
                ...
                {
                    txtBuscar.Text = "BUSCAR SERVICIOS";
                    txtBuscar.ForeColor = Color.Gray;
                }
            };

            btnBuscar.Click += (s, e) => BuscarEnCategoria(txtBuscar.Text);
        }

        private void BuscarEnCategoria(string texto)
        {
            if (_categoriaActual == null) return;

            if (string.IsNullOrWhiteSpace(texto) || texto == "BUSCAR SERVICIOS")
            {
                // Mostrar todos
            ...
            var serviciosFiltrados = _servicios
                .Where(s => s.Titulo.Contains(texto, StringComparison.OrdinalIgnoreCase) ||
                           s.Descripcion.Contains(texto, StringComparison.OrdinalIgnoreCase) ||
                           s.Proveedor.Contains(texto, StringComparison.OrdinalIgnoreCase))
                .ToList();
